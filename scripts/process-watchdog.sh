...
# Source of truth lives here; install.sh copies it to ~/.hermes/scripts/.
# Uses start-daemon.sh (the standard daemon), NOT legacy auto-start.sh.

PORT=8050
TMUX_SESSION="mem0"
DAEMON="${HOME}/.mem0-server/start-daemon.sh"
...
if tmux has-session -t "${TMUX_SESSION}" 2>/dev/null; then
    echo "mem0 port ${PORT} not responding, killing stale tmux session..."
